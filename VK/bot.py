...
import time
import re
from VK.bot_backend import VkSearcher
from settings import grp_token, app_id
...
class VkBot:
    ...
    def _hello_msg(self):
        ...
        text = 'Привет, чтобы найти пару' \
               'Набери в ответ OK чтобы начать'
        ...
        if prev_greeting['count'] != 0:
            ...
            text = 'Вижу ты уже пользовался ботом. Для продолжения пиши ОК' if delta_time < 600 else text
        return text
    ...
    def start_bot(self):
        ...
        for event in self.server_pool.listen():
            ...
            if step_action == 0:
                self._write_msg(self.user_id, self._hello_msg())
                step_action += 1
            ...
            if step_action == 1:
                ...
                if user_msg == "ок":
                    self._write_msg(self.user_id,
                                    ...
                                    f'https://oauth.vk.com/authorize?client_id={app_id}&display=page&scope='
            ...
            if step_action == 2:
                ...
                user1 = VkSearcher(token_user, user_id_num=self.user_id)
                step_action += 1
                user_msg = ""
...
if __name__ == '__main__':
    session = VkBot(token_bot=grp_token)
    session.start_bot()
...
from database_lib.create_database import create_db
...
class SqlDataPersons:
    def __init__(self, sql_name_database, user_data=None, person_data=None):
        ...
        self.engine = sq.create_engine(sql_name_database)
        self.connection = self.engine.connect()
        self.metadata = sq.MetaData(self.connection)

    def _find_column_in_table(self, table_name):
        t = sq.Table(f"{table_name}", self.metadata, autoload=True)
    ...
    def _get_relation(self, person_id=None):
        if person_id is None:
            return self.connection.execute(f'''SELECT clientid,personid 
                                               FROM usersconnect
                                               WHERE clientid={self.user_data['id']}''').fetchall()
        else:
            return self.connection.execute(f'''SELECT clientid,personid 
                                               FROM usersconnect
                                               WHERE clientid={self.user_data['id']} AND personid={person_id}''').fetchall()
